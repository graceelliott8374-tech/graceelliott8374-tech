# Grace Elliott

Full Stack MERN Developer | Psychology Student | Building practical software that solves real-world problems.

## About Me

I'm a Full Stack MERN developer and Psychology student concentrating in Addiction Studies at Southern New Hampshire University. I enjoy building practical software for small businesses, compliance, and behavioral health while continually expanding my skills in modern web development.

## Research & Active Development

- **Personal Portfolio** – A React portfolio showcasing my technical projects, education, and professional experience.
- **TraceLayer** – A documentation and asset-tracking platform designed to streamline warehouse operations through structured data capture, inventory documentation, and project tracking.
- **FractalPhoenix** – A research software project focused on multi-layered computational modeling. It provides a framework for representing complex systems as interacting layers, allowing relationships, constraints, and emergent behaviors to be analyzed in a structured and extensible way.
- **ComplianceSphere** – A compliance platform that validates employee benefits documentation against evidence-based regulatory requirements to improve accuracy, consistency, and audit readiness.

## Tech Stack

<p align="left">
  <img src="https://skillicons.dev/icons?i=js,python,html,css,react,nodejs,express,mongodb,git,github,vscode,vite,npm" />
</p>

## Education

**Southern New Hampshire University**  
B.A. Psychology, Concentration in Addiction | **GPA: 4.0** *(In Progress)*

**Persevere**
Full Stack MERN Software Development Program

## Areas of Interest

- Full Stack Web Development
- Research Software
- Computational Modeling
- Compliance Technology
- Behavioral Health
- Evidence-Based Decision Support

---

---

## Meet Tyger 🐈

<p align="center">
  <img src="tyger-bowtie.png" alt="Tyger, Chief Quality Assurance Officer" width="325">
</p>

<p align="center"><strong>Chief Quality Assurance Officer</strong></p>
<p align="center"><em>"I have reviewed this repository and find it acceptable."</em></p>

No development project is complete without Tyger's approval. He has supervised countless hours of coding, reviewed every workstation for maximum comfort, and maintains exceptionally high standards for morale, keyboard accessibility, and treat distribution.

### Experience
- 8+ years of uninterrupted supervision
- Keyboard testing specialist
- Professional lap warmer
- Conducts mandatory ergonomic inspections
- Performs pre-release quality assurance inspections

### Core Competencies
- 🐾 Keyboard Occupancy
- 🐾 Bug Detection (mostly insects)
- 🐾 Morale Support
- 🐾 Code Review by Observation
- 🐾 Lap Warming
- 🐾 Deadline Enforcement (dinner only)

### Certifications
- 🏅 Certified Red Dot Chaser
- 📦 Advanced Box Inspection
- 🥇 Senior Treat Acceptance Specialist

