# Grace Elliott

**Full Stack Developer | Psychology Student | Building research and educational software**

## About Me

I'm a Full Stack Developer and Psychology student concentrating in Addiction Studies at Southern New Hampshire University. My work increasingly focuses on the intersection of **psychology, research, and technology**.

I build full-stack applications using JavaScript, TypeScript, Python, React, Node.js, Express, FastAPI, MongoDB, and PostgreSQL. My current development work focuses on research technology, cognitive psychology, educational software, and tools for organizing and working with scientific evidence.

## Current Development

### EvidenceLens

A research-focused evidence platform designed to help researchers discover, organize, connect, and analyze academic literature.

**Technologies:** React, TypeScript, Vite, Python, FastAPI, PostgreSQL, SQLAlchemy, Alembic

**Current development includes:**
- Research literature discovery and organization
- Evidence management
- Research knowledge mapping
- Structured research data
- Researcher-controlled data export
- Search and discovery infrastructure

---

### CogPsyLab

A cognitive psychology learning and research platform designed for students and professors.

Students can complete assigned learning and experimental modules, while professors can manage assignments and review collected data.

**Technologies:** React, JavaScript, Node.js, Express

**Current development includes:**
- Student and professor account systems
- Assigned experimental modules
- Cognitive psychology lessons
- Knowledge checks
- Research methods instruction
- Experimental data collection
- Professor-facing research and assignment tools

---

### CV Portfolio

A responsive personal portfolio designed to present my software development work, education, technical skills, certifications, and professional experience.

**Technologies:** React, JavaScript, Vite, Node.js, Express

The portfolio is being designed as a scalable professional CV with individual project pages, live demonstrations, GitHub integration, and expanded documentation of both completed and ongoing development work.

## Tech Stack

### Languages & Core Web

<p>
  <img src="https://img.shields.io/badge/JavaScript-20232A?style=for-the-badge&logo=javascript&logoColor=F7DF1E" alt="JavaScript">
  <img src="https://img.shields.io/badge/TypeScript-20232A?style=for-the-badge&logo=typescript&logoColor=3178C6" alt="TypeScript">
  <img src="https://img.shields.io/badge/Python-20232A?style=for-the-badge&logo=python&logoColor=3776AB" alt="Python">
  <img src="https://img.shields.io/badge/HTML5-20232A?style=for-the-badge&logo=html5&logoColor=E34F26" alt="HTML5">
  <img src="https://img.shields.io/badge/CSS3-20232A?style=for-the-badge&logo=css&logoColor=1572B6" alt="CSS3">
</p>

### Frontend Development

<p>
  <img src="https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB" alt="React">
  <img src="https://img.shields.io/badge/Vite-20232A?style=for-the-badge&logo=vite&logoColor=646CFF" alt="Vite">
</p>

### Backend & APIs

<p>
  <img src="https://img.shields.io/badge/Node.js-20232A?style=for-the-badge&logo=node.js&logoColor=5FA04E" alt="Node.js">
  <img src="https://img.shields.io/badge/Express-20232A?style=for-the-badge&logo=express&logoColor=FFFFFF" alt="Express">
  <img src="https://img.shields.io/badge/FastAPI-20232A?style=for-the-badge&logo=fastapi&logoColor=009688" alt="FastAPI">
</p>

### Databases & Data

<p>
  <img src="https://img.shields.io/badge/MongoDB-20232A?style=for-the-badge&logo=mongodb&logoColor=47A248" alt="MongoDB">
  <img src="https://img.shields.io/badge/PostgreSQL-20232A?style=for-the-badge&logo=postgresql&logoColor=4169E1" alt="PostgreSQL">
  <img src="https://img.shields.io/badge/SQLAlchemy-20232A?style=for-the-badge&logo=sqlalchemy&logoColor=D71F00" alt="SQLAlchemy">
</p>

### Development Tools

<p>
  <img src="https://img.shields.io/badge/Git-20232A?style=for-the-badge&logo=git&logoColor=F05032" alt="Git">
  <img src="https://img.shields.io/badge/GitHub-20232A?style=for-the-badge&logo=github&logoColor=FFFFFF" alt="GitHub">
  <img src="https://img.shields.io/badge/VS_Code-20232A?style=for-the-badge&logo=visualstudiocode&logoColor=007ACC" alt="Visual Studio Code">
  <img src="https://img.shields.io/badge/npm-20232A?style=for-the-badge&logo=npm&logoColor=CB3837" alt="npm">
  <img src="https://img.shields.io/badge/Insomnia-20232A?style=for-the-badge&logo=insomnia&logoColor=4000BF" alt="Insomnia">
</p>

## Education

### Southern New Hampshire University

**B.A. Psychology, Concentration in Addiction**  
**GPA: 4.0** | In Progress

Academic interests include cognitive psychology, research methodology, human behavior, addiction, and the psychological implications of emerging technologies.

### Persevere

**Full Stack MERN Software Development Program**

Training in full-stack application development using MongoDB, Express, React, Node.js, JavaScript, HTML, CSS, Git, GitHub, and related web technologies.

## Areas of Interest

- Research Software & Research Technology
- Full Stack Software Development
- Cognitive Psychology
- Human-Computer Interaction
- Artificial Intelligence & Human-AI Interaction
- Research Methodology
- Evidence-Based Research Tools
- Educational Technology
- Data Organization & Visualization
